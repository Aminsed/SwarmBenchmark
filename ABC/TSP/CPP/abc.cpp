...
void initializeBees(Bee* bees, int* globalBestPosition, double* globalBestFitness, std::mt19937& rng) {
...
void sendEmployedBees(Bee* bees, int* globalBestPosition, double* globalBestFitness, std::mt19937& rng) {
    for (int tid = 0; tid < NUM_BEES; tid++) {
        Bee* b = &bees[tid];
...
void sendOnlookerBees(Bee* bees, int* globalBestPosition, double* globalBestFitness, std::mt19937& rng) {
    for (int tid = 0; tid < NUM_BEES; tid++) {
...
void sendScoutBees(Bee* bees, int* globalBestPosition, double* globalBestFitness, std::mt19937& rng) {
    for (int tid = 0; tid < NUM_BEES; tid++) {
        Bee* b = &bees[tid];
...
void runABC(Bee* bees, int* globalBestPosition, double* globalBestFitness) {
    ...
    for (int iter = 0; iter < MAX_ITERATIONS; iter++) {
        sendEmployedBees(bees, globalBestPosition, globalBestFitness, rng);
        sendOnlookerBees(bees, globalBestPosition, globalBestFitness, rng);
        sendScoutBees(bees, globalBestPosition, globalBestFitness, rng);
    }
}
...
int main() {
    ...
    initializeBees(bees, globalBestPosition, &globalBestFitness, rng);
