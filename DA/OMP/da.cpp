...
void updateDragonflies(Dragonfly* dragonflies, double* globalBestPosition, double* globalBestFitness) {
    ...
    std::uniform_real_distribution<double> enemyDis(-5.0, 5.0);
    ...
    for (int i = 0; i < NUM_DRAGONFLIES; i++) {
        ...
        for (int j = 0; j < DIMENSIONS; j++) {
            double r = dis(gen);
            d->step[j] = SEPARATION_WEIGHT * (globalBestPosition[j] - d->position[j]) +
                         ALIGNMENT_WEIGHT * d->step[j] +
                         COHESION_WEIGHT * (globalBestPosition[j] - d->position[j]) +
                         FOOD_ATTRACTION_WEIGHT * (globalBestPosition[j] - d->position[j]) +
                         ENEMY_DISTRACTION_WEIGHT * (enemyDis(gen) - d->position[j]);
